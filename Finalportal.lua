...
RunService.Heartbeat:Connect(function()
  ...
  for _, obj in pairs(workspace:GetChildren()) do
    ...
    local hum = obj:FindFirstChildOfClass("Humanoid")if hrp and hum and not teleportCooldowns[hrp] then
      -- Проверка касания Портала 1 -> Выход из Портала 2
      if (hrp.Position - p1.Position).Magnitude <= 3.8 then
        teleportCooldowns[hrp] = true
        
        if currentSkin == "EvilMorty" and targetPlaceId and Players:GetPlayerFromCharacter(obj) == LocalPlayer then
          TeleportService:Teleport(targetPlaceId, LocalPlayer) -- Телепорт в другой плейс!
        else
          local exit = p2.CFrame * CFrame.new(0, 0, -4)
          hrp.CFrame = CFrame.new(exit.Position, exit.Position + p2.CFrame.LookVector)
        end
        
        task.wait(1.2)
        teleportCooldowns[hrp] = nil

      -- Проверка касания Портала 2 -> Выход из Портала 1
      elseif (hrp.Position - p2.Position).Magnitude <= 3.8 then
        teleportCooldowns[hrp] = true
        
        local exit = p1.CFrame * CFrame.new(0, 0, -4)
        hrp.CFrame = CFrame.new(exit.Position, exit.Position + p1.CFrame.LookVector)
