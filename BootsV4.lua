--[[
	...
	При запуске игры сразу появляется предмет "Boots" — только сами ботинки,
	без надписей, без рук, без фона.

	Управление кликом/тапом по ботинкам (на игрока):
	  клик 1  -> надел ботинки
	  клик 2  -> включил антигравитацию (значок на ремне светится синим) —
	             можно ходить по стенам, потолкам, крышам — по любой
	             твёрдой поверхности, кроме пустого воздуха
	  клик 3  -> выключил (ботинки остаются надеты)
	  клик 4  -> снова включил
	  ...и так по кругу (вкл/выкл), после того как один раз надел.

	Никаких LocalScript — всё в одном обычном Script, работает на сервере.
--]]
...
local function onBootsClicked(player)
	...
	elseif d.state == 1 then
		...
		d.state = 1 -- выключил (остаётся надето); следующий клик снова включит
	end

	applyState(player, d.state)
end
...
local function findSurfaceNormal(character, rootPart, currentNormal)
	...
	return nil, nil -- рядом нет ни одной поверхности (открытый воздух)
end
	...
	for player, d in pairs(playerData) do
		if d.state == 2 then
			...
			if character and rootPart and humanoid and forces.alignPosition and humanoid.Health > 0 then
				...
				if normal then
					-- рядом есть стена/потолок/пол — прилипаем и идём по нему
				...
				-- normal == nil -> рядом только воздух, ничего не держим,
				-- персонаж падает под обычной гравитацией, пока не найдёт поверхность
			end
		end
	end
end)
...
Players.PlayerAdded:Connect(function(player)
	...
	player.CharacterAdded:Connect(function()
		local d = getData(player)
		task.wait(0.2) -- дать телу полностью загрузиться
		applyState(player, d.state)
	end)
end)
