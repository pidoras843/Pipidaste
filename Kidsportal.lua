...
 --[[
    PortalKidsTool.client.lua
    Положить в: Tool "Portal Kids" -> этот LocalScript
    Это упрощённая "детская" версия пушки (визуально ярче/меньше),
    механика та же: 2 портала = 1 шарик, максимум 10 шариков.
 
    Использует ту же колбу-модель, что и обычная пушка (можно поменять цвета).
--]]
...
local function buildKidFlask(handle)
    if handle:FindFirstChild("MiniFlask") then return end
 
    local flask = Instance.new("Part")
    flask.Name = "MiniFlask"
    flask.Shape = Enum.PartType.Cylinder
    flask.Size = Vector3.new(0.4, 0.35, 0.35) -- ещё меньше, "детская"
    flask.Material = Enum.Material.Glass
